...
contract Pharma {
    ...
    Roles role;
    event OwnerChange(string drugName, uint256 serialNo, uint256 from, uint256 to );
    mapping (address => Company) companies;
    mapping (string => mapping (uint256 => Drug)) public drugs;
     ...
     */
    function addDrug(string memory _name, uint256 _serialNo, uint256 _mfgDate, uint256 _expDate) public onlyCompany(msg.sender) returns (bool) {
        uint256 CRN = companies[msg.sender].CRN;
        require(CRN == companies[msg.sender].CRN, "Company Not Registered");
        require(companies[msg.sender].role == Roles(0), "You must be manufacturer");

        bytes32 drugId = bytes32(sha256(abi.encodePacked(_name, _serialNo))); 

        drugs[_name][_serialNo] = Drug(drugId, _name, _serialNo, CRN, _mfgDate, _expDate, CRN, 0);
        drugsByAddress[msg.sender].push(Drug(drugId, _name, _serialNo, CRN, _mfgDate, _expDate, CRN, 0));
        return true;
    }

    function getDrugs(string memory _drugName, uint256 _serialNo) public view returns(Drug memory) {
    ...
    function createPO( uint256 _sellerCRN, string memory _drugName, uint256 _quantity) public onlyCompany(msg.sender) returns (bool) {
        ...
        require (buyerRole != Roles(0), "Manufacturer cannot create purchase order");
        require (buyerRole != Roles(3), "Transporter cannot create purchase order");
        require(buyerCRN != _sellerCRN, "Buyer cannot be a seller");

        // TODO: 
        // if (buyerRole == Roles(1)) {
        //     // seller role must be 0
        //     // require ();
        // } else if (buyerRole == Roles(2)) {
        //     // seller role must be 1
        // } else if (buyerRole == Roles(3)) {
        //     // seller role must be 2
        // }
    ...
    // 12345, abc, 123456 
    function createShipment(uint256 _buyerCRN, string memory _drugName, uint256 _serialNo, uint256 _transporterCRN) public onlyCompany(msg.sender) returns (bool) {
        ...
        require (purchaseOrders[_buyerCRN].sellerCRN == ownerCRN, "Order Not Found");
        require (sellerRole != Roles(3), "Transporter cannot create shipment order");
        // TODO: check whether shipment is called or not
        // Check whether drugname is same or not
        require (sha256(abi.encodePacked(purchaseOrders[_buyerCRN].drugName)) == sha256(abi.encodePacked(_drugName)), "Order Not Found");
        ...
        // shipments[_transporterCRN].push(Shipment(id, ownerCRN, _drugName, _serialNo, quantity, _transporterCRN, Status(0)));
        shipments[_transporterCRN] = Shipment(id, ownerCRN, _drugName, _serialNo, quantity, _transporterCRN, Status(0));
    ...
    function updateShipment(uint256 _buyerCRN, string memory _drugName) public returns (bool) {
        ...
        require(companies[msg.sender].CRN != _buyerCRN, "Buyer can't update shipment");
        require(companies[msg.sender].role == Roles(3), "Only transporter can update shipment");
        require(companies[msg.sender].CRN == shipments[transporterCRN].transporter, "Transporter Not Found");
        require(shipments[transporterCRN].transporter == transporterCRN, "You do not have any shipment");
        if (sha256(abi.encodePacked(shipments[transporterCRN].drugName)) == sha256(abi.encodePacked(_drugName))) {
            require(shipments[transporterCRN].status == Status(0), "You do not have any shipment to update");
    ...
    function getShipments() public onlyCompany(msg.sender) view returns(Shipment memory) {
     ...
     */
    function retailDrug(string memory _drugName, uint256 _serialNumber, uint256 _customerAadhar) public onlyCompany(msg.sender) returns (bool) {
        uint256 CRN = companies[msg.sender].CRN;
        uint256 currentOwner = drugs[_drugName][_serialNumber].currentOwner;
        require (companies[msg.sender].role == Roles(2), "You are not a retailer");
     ...
     */
    // function viewHistory(string memory _drugName, uint256 _serialNo) public view returns (Drug memory) {
    //     // return OwnerChang
    // }
     ...
     * @param _serialNo uint256
     */
    function viewDrugCurrentState(string memory _drugName, uint256 _serialNo) public view returns (Drug memory) {
    //     // uint256 currentOwner = drugs[_drugName][_serialNo].currentOwner;
    //     // Roles currentOwnerRole = companies[currentOwner].role;
    //     // if (sha256(abi.encodePacked(drugs[_drugName][_serialNo].name)) == sha256(abi.encodePacked(_drugName)) && drugs[_drugName][_serialNo].serialNo == _serialNo) {
    //     //     if (currentOwnerRole == Roles(0)) {
